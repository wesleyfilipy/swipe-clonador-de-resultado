...
export function getCreativeVideoSrc(
  ad: Pick<AdRow, "video_storage_path" | "video_url" | "vsl_url" | "creative_url">
): string | null {
  ...
  if (ad.creative_url && isDirectVideoUrl(ad.creative_url)) {
    return toMediaProxyUrl(ad.creative_url) ?? ad.creative_url;
  }
...
export function hasCreativoFile(
  ...
  if (ad.creative_url && isDirectVideoUrl(ad.creative_url)) return true;
...
export function getCreativoDownloadSourceUrl(
  ...
  if (ad.creative_url && isDirectVideoUrl(ad.creative_url)) return ad.creative_url;
...
export function hideDuplicateVslSplit(
  ...
  const hasDedicatedCreative = Boolean(
    ad.video_storage_path ||
      (ad.video_url && ad.video_url.length > 0) ||
      (ad.creative_url && isDirectVideoUrl(ad.creative_url))
  );
